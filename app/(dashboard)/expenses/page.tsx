        ...
        <div className="grid gap-5">
          {expenses.map((expense) => (
            ...
            >
              ...
              {/* Submit Button only for Draft and owned by current user */}
              {expense.status === "DRAFT" && expense.userId === user.userId && (
                <SubmitExpenseButton id={expense.id} />
              )}

              {expense.status === "DRAFT" && expense.userId !== user.userId && (
                <p className="mt-4 text-sm text-gray-500">
                  Only the owner can submit this draft.
                </p>
              )}
            </div>
          ))}
